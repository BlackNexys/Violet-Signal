...
try {
  ...
  await page.getByRole('combobox', { name: 'Sound patch' }).selectOption('chrome-wound/razor-assembly@1')
  ...
  await page.getByRole('combobox', { name: 'Sound patch' }).selectOption('fractured-relay/wire-below@1')
  ...
  await page.getByRole('combobox', { name: 'Sound patch' }).selectOption('chrome-wound/iron-pulse@1')
  await page.getByLabel('Voice controls').getByRole('button', { name: 'Texture', exact: true }).click()
  await page.getByRole('combobox', { name: 'Sound patch' }).selectOption('chrome-wound/arc-ash@1')
  await page.getByText('Bright metallic debris in a noise cloud').waitFor()
  const expandedEngineCode = await editor.innerText()
  expect(expandedEngineCode.includes('patch chords: chrome-wound/razor-assembly@1'), 'dual patch did not reach code')
  expect(expandedEngineCode.includes('patch bass: fractured-relay/wire-below@1'), 'pluck patch did not reach code')
